...
import { Table, Button, Form, Input, Select, Space, message, Tabs, Card } from 'antd';
...
export default function FinanzasView() {
  ...
  const onFinishTransferencia = async (values: any) => {
    try {
      ...
      message.error('Error al realizar transferencia');
    }
  };
  ...
  const items = [
    {
      ...
      children: (
        <Table
          ...
          rowKey="id"
          columns={[
            { title: 'Fecha', dataIndex: 'fecha', render: (f: any) => (f ? `${f[2]}/${f[1]}/${f[0]}` : '-') },
            {
              title: 'Descripción',
              ...
              render: (text: string) => {
                if (text?.includes('Transferencia')) {
                  ...
                  if (ids && ids.length >= 2) {
                    const origen = cuentas.find((c) => c.id.toString() === ids[0])?.alias || ids[0];
                    const destino = cuentas.find((c) => c.id.toString() === ids[1])?.alias || ids[1];
                    return `Transferencia de cuenta ${origen} a ${destino}`;
                  }
                }
                return text;
              },
        ...
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <Table
            ...
            columns={[
              ...
              { title: 'Saldo', dataIndex: 'saldoBase' },
            ]}
          />
        </Space>
      ),
